...
def docs_page(doc_path: str) -> FileResponse:
    return _serve_project_file(DOCS_DIR, doc_path)


@app.get("/README.md", include_in_schema=False)
def readme() -> FileResponse:
    return FileResponse(PROJECT_ROOT / "README.md", media_type="text/markdown; charset=utf-8")


@app.get("/deployment", include_in_schema=False)
def deployment_index() -> FileResponse:
    return FileResponse(DEPLOYMENT_DIR / "index.html")


@app.get("/deployment/{doc_path:path}", include_in_schema=False)
def deployment_page(doc_path: str) -> FileResponse:
    return _serve_project_file(DEPLOYMENT_DIR, doc_path)


def _serve_project_file(root: Path, doc_path: str) -> FileResponse:
    file_path = (root / doc_path).resolve()
